...
class OverthecapSpider(scrapy.Spider):
    ...
    def parse_contract_details(self, response):
        print(f"Parsing contract details for {response.meta.get('player_name', 'Unknown')}")
    ...
    def parse_active_2024(self, years_active_string):
        ...
        if len(years_active_arr) == 1:
            ...
            end_year = int(years_active_arr[1].strip()) if years_active_arr[1].strip() else 2024
    ...
    def parse_contracts(self, response):
        earnings_table = response.xpath('//table[@class="career-earnings-position position-table sortable"]')
        ...
        for row in rows:
            ...
            if link:
                ...
                player_name = row.xpath('.//td[1]//a/text()').get().strip().replace(',', '')
                ...
                if is_active_2024 == True:
                    yield scrapy.Request(full_url, callback=self.parse_contract_details, meta={'player_name': player_name})
                else:
                    print(f"Player {player_name} is not active in 2024, skipping contract details.")
                    continue
            else:
                continue
